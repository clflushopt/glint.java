...
// In-memory implementation of a data source.
public class MemoryDataSource implements DataSource {
    ...
    public List<RecordBatch> scan(List<String> projection) {
        var projectionIndices = projection.stream().map(
                name -> Iterables.indexOf(schema.getFields(), field -> field.name() == name))
                .collect(Collectors.toList());

        return records.stream()
                .map(batch -> new RecordBatch(schema, projectionIndices.stream().map(index -> batch.getField(index))
                        .collect(Collectors.toList())))
                .toList();
    }
}
